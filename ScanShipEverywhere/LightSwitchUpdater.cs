...
public class LightSwitchUpdater : MonoBehaviour {
    ...
    private void Awake() =>
        _scanNodeProperties ??= GetComponentInChildren<ScanNodeProperties>();


    private void LateUpdate() {
        if (_scanNodeProperties is null)
            return;
        ...
        if (_scanNodeProperties.enabled == localPlayer.isInHangarShipRoom)
            return;
